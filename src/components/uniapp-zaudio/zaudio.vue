		...
		<template v-if="theme == 'theme1'">
          ...
          <image :src="renderData('coverImgUrl') || require('./static/default-cover.jpeg')" mode="aspectFill" class="cover" :class="{ on: !renderData('paused') }"></image>
			...
			<view class="audio-button-box">
				<!-- 块退15s -->
				<image :src="require('./static/prev.png')" class="prevbtn" @click="stepPlay(-15)"></image>
				<!-- 上一首 -->
				<image :src="require('./static/go.png')" class="prevplay" @click="changeplay(-1)"></image>
				<div class="playbox">
				...
				<!-- 下一首 -->
				<image :src="require('./static/go.png')" class="nextplay" @click="changeplay(1)"></image>
				<!-- 快进15s -->
				<image :src="require('./static/next.png')" class="nextbtn" @click="stepPlay(15)"></image>
			</view>
		</template>
	</view>
</template>
